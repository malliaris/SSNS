...
<p>The input field includes the current value ($$1.2$$, here) and, on the left, the parameter's symbol ($$T$$) on a colored background.  The color indicates when a changed value takes effect &mdash; <span class="ctrl_eff_immed px-2 py-0 mr-1">blue</span><strong>:</strong> immediately, or at next recorded step; <span class="ctrl_eff_reload px-2 py-0 mr-1">green</span><strong>:</strong> a <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_C_RELOAD_CMDS'); ">reload</a> is needed.  A system type might also have one or more <span class="ctrl_precip_reload px-2 py-0 mr-1">grey</span> "meta-parameter" buttons.</p>

<p><strong><em>Any of the colored symbol backgrounds in the</em></strong> <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_PRMS_DROPDOWN'); "><samp>PRMS</samp></a> <em><strong>and</em></strong> <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_CTRL_DROPDOWN'); "><samp>CTRL</samp></a> <em><strong>dropdown areas can be clicked to view the corresponding quantity's help page</em></strong>.</p>

<p>The <span class="ctrl_precip_reload px-2 py-0 mr-1">grey</span> "meta-parameter" buttons generally handle "settings" that are more complicated than parameter values.  For instance, the <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_ST_HS'); ">hard sphere gas</a> has an <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Initial_condition">initial condition</a> ($$\mathrm{IC}$$) button that determines the initial positions and velocities of all gas particles.  As it's clicked, each "meta-parameter" button cycles through a fixed number of options and displays description / indicator / icon like so: $$\mathrm{description}$$ <span><img height="30" width="6" src="/static/images/SSNS/UICI/cycle_indicator_5_2.svg">&#128070;</span></p>
...
<li>button input, discussed e.g., <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_C_ONE_STEP_CMDS'); ">here</a>; specialized input buttons called $$\texttt{UICI}$$s are described <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_PRMS_DROPDOWN'); ">here</a></li>
...
<li>The <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_CODE_ORGANIZATION'); "><samp>Coords</samp></a> <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Computer_memory">memory footprint</a> for most <strong>SSNS</strong> system types is small.  As <a target="_blank" class="hv_ext_link" href="//github.com/malliaris/SSNS/blob/main/README.md#general-timeenergyvisualizationcomputation-considerations">this discussion</a> on GitHub points out, this is not the case for the <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_SA_SM'); ">statistical mechanical</a> models, and several strategies have been or could be pursued to improve their computational performance/capabilities.  For example, it may be worth extending the <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Monte_Carlo_method">Monte Carlo</a> move set for our <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_SPIN_SYSTEMS'); ">spin systems</a> beyond the single Metropolis spin flip.</li>
...
<p>If the <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Time_evolution">next state</a> of a <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Stochastic_process">stochastic process</a> is described by a <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Probability_distribution">probability distribution</a>, it is reasonable to ask what that probability distribution depends on.  It may:</p>
...
<li>We do keep some 3D terminology (hard <em>spheres</em>, not disks), but the volume $$V$$ is really an area and pressure $$p$$ has units of force/length so $$p V$$ represents <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Work_(physics)#Work_by_a_gas">work/energy</a>.</li>
...
<li>For simplicity, we skip the <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Thermal_reservoir">heat bath</a> and assume <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Adiabatic_process">adiabatic</a> conditions (i.e., well insulated walls).  All collisions are <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Elastic_collision">elastic</a> and instantaneous, so <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Interatomic_potential">potential energy</a> plays no role and the <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Internal_energy">total energy</a> $$ U = \frac{1}{2} \sum_i m_i v_i^2 $$.  Except for during piston movement in <samp>HS</samp>, $$U$$ is <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Conservation_law">fixed</a>.</li>
...
<li>For both, the parameter <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_P_SM_HS_kT0'); ">$$ k_B T_0 $$</a> controls the initial total energy.  If no sampling is involved, it will be exactly $$ N k_B T_0 $$.  However, for sampled initial conditions (e.g., equilibrium), there will be deviation from this value due to <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_P_SM_HS_N'); ">$$ N $$</a>-dependent <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Sampling_error">sampling error</a>.  That error, as well as possible piston movement in <samp>HS</samp>, mean that $$T \equiv U / N k_B$$ may differ from $$T_0$$.</li>
...
<p>The <strong>SSNS</strong> <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_ST_HS'); ">Hard Sphere</a> (<samp>HS</samp>) implementation &mdash; and, to a lesser extent, the <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_ST_IG'); ">ideal gas</a> (<samp>IG</samp>) implementation &mdash; can be thought of as a virtual laboratory apparatus; there are "dials" to adjust and quantities to "measure," so we can check various theoretical results.  Our comparisons below involve the usual $$p$$, $$V$$, $$N$$, and $$k_B T$$, as well as the <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Internal_energy">internal energy</a> $$U$$, <samp>HS</samp> particle radius $$R$$, <samp>HS</samp> <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Packing_density">area fraction</a> $$\eta$$, and <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Compressibility_factor">compressibility factor</a> $$Z = \frac{p V}{ N k_B T }$$.</p>
...
The variable $$ x(t) $$ represents the number of individuals that possess version $$ A $$ at time $$t$$.</p>
...
<p><em>Details:</em> Reaction systems are most naturally treated with a <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Discrete_time_and_continuous_time">continuous</a> time variable in terms of <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Survival_analysis#Hazard_function_and_cumulative_hazard_function">hazard functions</a> and, e.g., the <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Gillespie_algorithm">Gillespie algorithm</a>.  Since <strong>SSNS</strong> uses equi-spaced discrete time steps, we form <a target="_blank" class="hv_ext_link" href="//en.wikipedia.org/wiki/Markov_chain#Transitions">transition probabilities</a> $$ \alpha \Delta t $$ and $$ \beta x \Delta t $$, but must also ensure that these probabilities never exceed $$1$$.  We choose to <strong>(1)</strong> restrict $$ 10^{-4} \le \alpha \le 1 $$ and $$ 10^{-4} \le \beta \le 1 $$, <strong>(2)</strong> tentatively set $$\Delta t = 1$$, and finally <strong>(3)</strong> check at each time step, scaling both parameters if needed.  We scale by $$\frac{1}{ \alpha + \beta x_{\text{larg.}} }$$ where $$x_{\text{larg.}}$$ is the largest $$x$$ value currently present in the <a class="hv_link" onclick="window.sim.ui.hv.show_view('HV_C_SP_ENSEMBLE'); ">ensemble</a>.</p>
...
<tr><td><strong>RESERVED</strong>; e.g., for <a target="_blank" class="hv_ext_link" href="//github.com/malliaris/SSNS/blob/main/README.md#hard-sphere-gas-p-v-diagram-creation">this</a></td>
