...
document.getElementById('startSimulation').addEventListener('click', function() {
    ...
    const interval = setInterval(() => {
        ...
        for (let i = 0; i < particles.length; i++) {
            particles[i].updatePosition(timeStep, userBoxWidth, userBoxHeight, mode); // Pass mode to updatePosition
            console.log(`Atom ${i} Position: (${particles[i].x.toFixed(2)}, ${particles[i].y.toFixed(2)})`);
        }
