...
///////////////////////////////////////////////////////////////////////////////////////////////
////////                                                                                   ////
////////  * except for TrajSeg, each of these classes is the root of a class hierarchy     ////
////////  * see diagram SSNS_classes.pdf for the full inheritance/composition picture      ////
////////  * interface requirements are enforced in the javascript manner:                  ////
////////                                                                                   ////
////////  if (!this.METHOD) throw new Error("Derived CLASS must define METHOD()");         ////
////////                                                                                   ////
///////////////////////////////////////////////////////////////////////////////////////////////
...
class Trajectory {
    ...
    static get_t_0_from_UI() {
	return CU.gi("UI_CTRL_t_0");
    }
    static get_t_max_from_UI() {
	return CU.gi("UI_CTRL_t_max");
    }
	...
	this.t_0 = Trajectory.get_t_0_from_UI();  // should in principle check that input value can be stored sensibly in js Number, but skip for now
	let requested_t_max = Trajectory.get_t_max_from_UI();  // Javascript ensures in "real time" that t_max >= t_0 + 1, so no need to check for that
	...
	this.ia.ui.indicate_params_pushed_traj_to_UI();
    }
	...
	if ( ! this.at_t_0()) {  // if there's "room" to decrement...
	    ...
	    if (move_prev_seg || this.ia.rs.params_changed) {
	...
	if (! this.at_t_edge()) {  // if there's "room" to increment...
	    ...
	    if (move_next_seg || this.ia.rs.params_changed) {
		this.push_curr_Params_to_UI();  // MUST BE AFTER csi++ !
	    }
	...
	if (this.at_t_max()) {  // if there's no "room", i.e., t_edge == t_max, alert the user
	    alert("t_max reached!");  //////////// eventually use help lightbox for this
	    ...
	    if (this.ia.rs.params_changed) {  // if the parameters have been changed, we start a new segment...
	...
	if ((this.csi != orig_csi) || this.ia.rs.params_changed) {
	    this.push_curr_Params_to_UI();
	}
    }
    ...
    at_enabled_t_stop() {
	return (CU.gcb("UI_CTRL_use_t_stop") && (this.t == CU.gi("UI_CTRL_t_stop")));  // t_stop is enabled and t == t_stop
    }
}
