...
Many JavaScript variable names in the code use two-letter abbreviations to indicate category, subcategory, etc.  For example, when simulating with the Logistic Map (<samp>LM</samp>) system type, the derived <samp>Trajectory</samp> class being used is <samp>Trajectory_LM</samp>.  <samp>PlotType</samp> derived classes contain two such abbreviations, e.g., <samp>PlotTypeXT_LM</samp>; the first abbreviation corresponds to the intermediate level branches in the <samp>PlotType</samp> [class hierarchy](#classes-hierarchies-etc); the second abbreviation, to the leaf nodes.
