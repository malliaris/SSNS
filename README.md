...
A <samp>Trajectory</samp> object holds all the data that is generated as the system type's update equations are iterated in discrete time steps.  The object is <a href="/class_diagrams/Trajectory_composition_-_no_section_label.svg">composed</a> of a <samp>ModelCalc</samp> (which performs all model-specific calculations), one or more <samp>TrajSeg</samp> objects to hold the trajectory data, and supporting variables related to time, etc.

The <samp>TrajSeg</samp> represents the portion of a trajectory where the system evolved with a particular set of system parameter values.  It is thus <a href="/class_diagrams/TrajSeg_composition_-_no_section_label.svg">composed</a> of a single <samp>Params</samp> object and one or more <samp>Coords</samp> objects.

Each <samp>Coords</samp> object hold all dependent variable(s) data for a single time step &mdash; it could be a single scalar integer value, or a large 2D matrix of floating point values.   , ution allows for   es,  part of large class hierarchies
