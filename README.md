...
* When it comes to web implementations of the <samp>XY</samp> model, ours pales in comparison to the one found <a href="//kjslag.github.io/XY/">here</a>, and for many reasons.  One of the main reasons is that, despite always running on a device with a GPU, we do not take advantage of the GPU for our non-graphical computation!  GPU hardware WebGL  XY WebGPU GPGPU machine learning <samp>PRMS</samp> and <samp>CTRL</samp> a errors HTML 5 video element
